...
if ~exist(planFile, 'file')
    error('Plan file %s not found. Run sampling3.m first.', planFile);
end
...
if ~isfield(plan, 'display') || ~isstruct(plan.display)
    error('Plan file %s is missing display settings. Regenerate using sampling3.m.', planFile);
end
dp = plan.display;
...
if ~isfield(plan, 'dotParams') || ~isstruct(plan.dotParams)
    error('Plan file %s is missing dot configuration. Regenerate using sampling3.m.', planFile);
end
...
if ~isfield(plan, 'motionParams') || ~isstruct(plan.motionParams)
    error('Plan file %s is missing motion configuration. Regenerate using sampling3.m.', planFile);
end
motionParams = plan.motionParams;

if ~isfield(plan, 'timingParams') || ~isstruct(plan.timingParams)
    error('Plan file %s is missing timing configuration. Regenerate using sampling3.m.', planFile);
end
timingParams = plan.timingParams;

if ~isfield(plan, 'gridConfig') || ~isstruct(plan.gridConfig)
    error('Plan file %s is missing grid configuration. Regenerate using sampling3.m.', planFile);
end
...
try
    ...
    % ---- Display counter: treat each within-block segment as a "part" ----
    fracsGlobal = [];
    ...
    nPartsPerBlock = 1 + numel(fracsGlobal);  % e.g., [0.5] -> 2 parts per block
    nPartsTotal    = numel(plan.blocks) * nPartsPerBlock;
    partIdx        = 0;

    for b = 1:numel(plan.blocks)
        block = plan.blocks(b);
        partIdx = partIdx + 1;
        showBlockIntro(dp, sprintf('실험 블록 %d / %d', partIdx, nPartsTotal));
       ...
       ~isempty(timingParams.withinBlockBreakFractions)
        ...
        for t = 1:numel(block.trials)
            ...
            if timingParams.postTrialMs > 0
                abortExperiment = presentFixation(dp, timingParams.postTrialMs, kb);  % 색 인자 생략 → 흰색
                if abortExperiment
                    break;
                end
            end
    ...
    % ---- End screen (only when fully completed, not aborted) ----
    if ~abortExperiment
        showEndScreen(dp, kb);
    end
    Screen('CloseAll');
...
for b = 1:numel(practiceBlocks)
    ...
    showBlockIntro(dp, sprintf('연습 블록 %d / %d', b, numel(practiceBlocks)));
