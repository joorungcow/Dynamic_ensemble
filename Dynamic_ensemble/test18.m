...
if ~exist(planFile, 'file')
    error('Plan file %s not found. Run sampling4.m first.', planFile);
end
...
if ~isfield(plan, 'display') || ~isstruct(plan.display)
    error('Plan file %s is missing display settings. Regenerate using sampling4.m.', planFile);
end
dp = plan.display;
...
if ~isfield(plan, 'dotParams') || ~isstruct(plan.dotParams)
    error('Plan file %s is missing dot configuration. Regenerate using sampling4.m.', planFile);
end
...
if ~isfield(plan, 'motionParams') || ~isstruct(plan.motionParams)
    error('Plan file %s is missing motion configuration. Regenerate using sampling4.m.', planFile);
end
motionParams = plan.motionParams;

if ~isfield(plan, 'timingParams') || ~isstruct(plan.timingParams)
    error('Plan file %s is missing timing configuration. Regenerate using sampling4.m.', planFile);
end
timingParams = plan.timingParams;

if ~isfield(plan, 'gridConfig') || ~isstruct(plan.gridConfig)
    error('Plan file %s is missing grid configuration. Regenerate using sampling4.m.', planFile);
end
...
results.practiceTrials = repmat(initTrialResultStruct(), 0, 1);
...
try
    ...
    % --------- Hidden debug practice: P key ---------
    if strcmp(mode, 'practice')
        if isfield(plan, 'practiceBlocks') && ~isempty(plan.practiceBlocks)
            fprintf('Running hidden debug practice block 1 only...\n');
            fprintf('Hidden debug condition label: %s\n', plan.practiceBlocks(1).label);

            results.practiceTrials = runPracticeBlocks(dp, kb, plan, params, ...
                gridLayout, gridConfig, motionParams, timingParams, 1);

            showPracticeEndScreen(dp, kb, {
                sprintf('연습 점검이 끝났습니다. (조건: %s)', plan.practiceBlocks(1).label);
                '스페이스바를 누르면 종료합니다.'
            });
        else
            fprintf('No practiceBlocks found in plan. Skipping hidden debug practice.\n');
        end

        results.trials = repmat(initTrialResultStruct(), 0, 1);
    ...
    % --------- Main experiment with interleaved practice ---------
    ...
    % ---- Display counter: each within-block segment is a displayed block ----
    fracsGlobal = [];
    if isfield(timingParams,'withinBlockBreakFractions') && ~isempty(timingParams.withinBlockBreakFractions)
        fracsGlobal = timingParams.withinBlockBreakFractions(:).';
        fracsGlobal = fracsGlobal(fracsGlobal > 0 & fracsGlobal < 1);
        fracsGlobal = unique(fracsGlobal);
    end
    nPartsPerBlock = 1 + numel(fracsGlobal);   % [0.5] -> 2 displayed parts per block
    nPartsTotal    = numel(plan.blocks) * nPartsPerBlock;  % 3 blocks -> 6 displayed blocks
    partIdx        = 0;

    for b = 1:numel(plan.blocks)

        % ===== Practice before each main block =====
        if isfield(plan, 'practiceBlocks') && numel(plan.practiceBlocks) >= b && ...
                ~isempty(plan.practiceBlocks(b).trials)

            showBlockIntro(dp, kb, {
                sprintf('연습 %d / %d', b, numel(plan.practiceBlocks));
                sprintf('이번 연습 조건: %s', plan.practiceBlocks(b).label);
                '이제 연습 시행을 시작합니다.';
                ' ';
                '연습에서는 이전 시행의 정답 여부에 따라 고정점 색이 바뀝니다.';
                ' ';
                '정답은 초록, 오답은 빨강입니다.';
                ' ';
                '화면은 스페이스바로 진행합니다.';
            ...
            practiceOut = runPracticeBlocks(dp, kb, plan, params, ...
                gridLayout, gridConfig, motionParams, timingParams, b);

            if ~isempty(practiceOut)
                results.practiceTrials = [results.practiceTrials; practiceOut(:)];
            end

            showPracticeEndScreen(dp, kb, {
                sprintf('연습 %d / %d이 끝났습니다.', b, numel(plan.practiceBlocks));
                ' ';
                sprintf('이제 본 시행 블록 %d / %d부터 시작합니다.', (b-1)*nPartsPerBlock + 1, nPartsTotal);
                sprintf('이번 본 시행 조건: %s', plan.blocks(b).label);
                ' ';
                '본 시행에서는 피드백 없이 진행됩니다.';
                ' ';
                '준비가 되면 스페이스바를 눌러주세요.'
            });
        end
        ...
        partIdx = partIdx + 1;
        showBlockIntro(dp, kb, {
            sprintf('본 시행 블록 %d / %d', partIdx, nPartsTotal);
            sprintf('조건: %s', block.label);
            ' ';
            '가능한 한 빠르고 정확하게 응답해 주세요.';
            ' ';
            '1번 키: T1 평균이 더 큽니다';
            '2번 키: T2 평균이 더 큽니다';
            ' ';
        ...
        for t = 1:numel(block.trials)
            ...
            abortExperiment = presentFixation(dp, timingParams.isiDurationMs, kb);
            if abortExperiment, break; end
            ...
            if timingParams.postTrialMs > 0
                abortExperiment = presentFixation(dp, timingParams.postTrialMs, kb);
                if abortExperiment
                    break;
                end
            end

            abortExperiment = presentBlank(dp, timingParams.postTrialMs, kb);
            if abortExperiment
                break;
            end
...
resultStruct = struct( ...
    'trialIndex', [], ...initTrialResultStruct
    'comboLabel', '', ...
    'methodPair', '', ...
    'freqRatio', '', ...
    'diffLevelTarget', [], ...
    ...
    'didRespond', false, ...
    'correct', NaN, ...
    'testChosen', NaN, ...   % 1 = test interval 선택, 0 = standard 선택, NaN = no response
    'targetPSMeans', [], ...
    'isPractice', false); 
end

function showBlockIntro(dp, kb, lines)
if nargin < 3 || isempty(lines)
    lines = {''};
elseif ischar(lines) || isstring(lines)
    lines = cellstr(lines);
end
...
while true
    ...
    y = dp.cy - (numel(lines)-1)*25;
    for i = 1:numel(lines)
        txt = double(lines{i});
        bounds = Screen('TextBounds', dp.wPtr, txt);
        x = dp.cx - RectWidth(bounds)/2;
        Screen('DrawText', dp.wPtr, txt, x, y, dp.textColor);
        y = y + 40;
    end
    ...
    if kb.useKbQueueCheck
        ...
        if pressed && any(firstPress(kb.spaceKey))
            break;
        end
    else
...
lines = {
    '이 실험은 연습과 본 시행으로 구성되어 있습니다.';
    ' ';
    '안내 화면은 모두 스페이스바로 진행합니다.';
    ' ';
    '응답은 1번 키(T1이 더 큼), 2번 키(T2가 더 큼)입니다.';
    ' ';
    '중간중간 쉬는시간이 제공됩니다. 충분히 휴식해주세요.';
    ' ';
    '준비가 되면 스페이스바를 눌러 시작해주세요.';
    ' ';
};
...
function practiceResults = runPracticeBlocks(dp, kb, plan, params, gridLayout, gridConfig, motionParams, timingParams, blockIndices)

if nargin < 9 || isempty(blockIndices)
    blockIndices = 1:numel(plan.practiceBlocks);
end

practiceBlocks = plan.practiceBlocks(blockIndices);

if isempty(practiceBlocks)
    practiceResults = repmat(initTrialResultStruct(), 0, 1);
    return;
end
...
for b = 1:numel(practiceBlocks)
    ...
    for t = 1:numel(block.trials)
        ...
        if timingParams.postTrialMs > 0
            abortExperiment = presentFixation(dp, timingParams.postTrialMs, kb, fixColorNext);
            if abortExperiment
                break;
            end
        end
...
function showPracticeEndScreen(dp, kb, lines)
if nargin < 3 || isempty(lines)
    lines = {
        '연습 시행이 끝났습니다.';
        ' ';
...
% 빈도 패턴에 따른 base 세트 생성
freqBase = make_frequency_set(freqCounts, params);
eqBase   = make_equalFreq_set(eqCounts, params);

% methodPair에 따라 interval별로 FREQ/EQ 배정
switch trialSpec.methodPair
    case 'FREQ_vs_EQ'
        base1 = freqBase;  % T1: FREQ
        base2 = eqBase;    % T2: EQ
    case 'EQ_vs_FREQ'
        base1 = eqBase;    % T1: EQ
        base2 = freqBase;  % T2: FREQ
    otherwise
        error('Unknown methodPair: %s', trialSpec.methodPair);
end

% standard / test target mean (perceptual scale)
muStdTarget  = 1;
muTestTarget = 1 + trialSpec.diffLevel;

if ~isfield(trialSpec, 'standardInterval') || isempty(trialSpec.standardInterval)
    if trialSpec.signS >= 0
        standardInterval = 1;
    else
        standardInterval = 2;
    end
else
    standardInterval = trialSpec.standardInterval;
end

if standardInterval == 1
    testInterval = 2;
    targetMean1  = muStdTarget;
    targetMean2  = muTestTarget;
elseif standardInterval == 2
    testInterval = 1;
    targetMean1  = muTestTarget;
    targetMean2  = muStdTarget;
else
    error('standardInterval must be 1 or 2.');
end

% dot-level jitter는 synthesizeSetFromBase 내부에서 처리됨
[t1SizesDeg, t1MeanPS] = synthesizeSetFromBase(base1, targetMean1, params);
[t2SizesDeg, t2MeanPS] = synthesizeSetFromBase(base2, targetMean2, params);
...
% standard / test의 실제 meanPS
if standardInterval == 1
    standardMeanPS = t1MeanPS;
    testMeanPS     = t2MeanPS;
else
    standardMeanPS = t2MeanPS;
    testMeanPS     = t1MeanPS;
end

psDiffStandardTest   = (testMeanPS / standardMeanPS) - 1;
diffLevelObservedPS  = psDiffStandardTest * 100;
...
info.standardInterval   = standardInterval;
info.testInterval       = testInterval;
info.standardMeanPS     = standardMeanPS;
info.testMeanPS         = testMeanPS;
info.psDiffObserved     = psDiffStandardTest;    % test vs standard
info.diffLevelObservedPS = diffLevelObservedPS;

info.jitterPS            = 0;
info.targetPSMeans       = [muStdTarget muTestTarget];
info.psTargetsAfterJitter = info.targetPSMeans;
...
msgRest = double('쉬는시간 입니다. 15초 이상 휴식해주세요');
msgGo   = double('스페이스바를 누르면 이어서 시작합니다.');
...
startTime = GetSecs;
%deadline  = startTime + durationMs / 1000; % durationMs는 이제 시간 제한에 사용하지 않음 (forced-choice)
...
while ~response.wasAborted && ~response.didRespond
    Screen('FillRect', dp.wPtr, dp.bkColor);
...
trialResult.standardInterval = stimInfo.standardInterval;
trialResult.testInterval     = stimInfo.testInterval;
trialResult.standardMeanPS   = stimInfo.standardMeanPS;
trialResult.testMeanPS       = stimInfo.testMeanPS;
...
if trialResult.didRespond
        if trialResult.testInterval == 1
            trialResult.testChosen = double(strcmp(trialResult.responseChoice, 'T1'));
        elseif trialResult.testInterval == 2
            trialResult.testChosen = double(strcmp(trialResult.responseChoice, 'T2'));
        else
            trialResult.testChosen = NaN;   
        end
    else
        trialResult.testChosen = NaN;       % 미응답 trial
end
...
msg = '실험이 모두 끝났습니다. 감사합니다. 연구자를 불러주세요.';  % 1줄
msg = double(msg);
...
while true
    if kb.useKbQueueCheck
        [pressed, firstPress] = KbQueueCheck;
        if pressed
            if any(firstPress(kb.escKey)) || any(firstPress(kb.spaceKey))
                break;
            end
        end
    else
        [keyIsDown, ~, keyCode] = KbCheck(-1);
        if keyIsDown
            if keyCode(kb.escKey) || keyCode(kb.spaceKey)
                break;
            end
        end
    end
