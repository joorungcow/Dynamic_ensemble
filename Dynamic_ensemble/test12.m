...
dotParams.meanDiffTolerance = 0.005;       % 지각 척도 평균 차이 허용 오차
...
saveFileName = sprintf('results_test12');
...
try
    ...
    totalTrials = numel(order);
    restAfterTrials = round((1:2) * totalTrials / 3);
    restAfterTrials = unique(restAfterTrials);
    restAfterTrials(restAfterTrials <= 0 | restAfterTrials >= totalTrials) = [];
    ...
    for idx = order
        ...
        [t1TargetMeanDeg, t2TargetMeanDeg] = jitterMeanPair( ...
            t1TargetMeanDeg, t2TargetMeanDeg, cond.diffLevel, dotParams);
        ...
        completedTrials = trialCounter;
        trialCounter = trialCounter + 1;

        if ~abortExperiment && ismember(completedTrials, restAfterTrials)
...
function [t1MeanDeg, t2MeanDeg] = jitterMeanPair(baseT1Deg, baseT2Deg, diffLevel, params)
maxAttempts = 1000;
expo = params.perceptualExponent;
minDeg = params.minSizeDeg;
maxDeg = params.maxSizeDeg;
tolerance = params.meanDiffTolerance;

targetSign = sign(baseT2Deg^expo - baseT1Deg^expo);
if targetSign == 0
    targetSign = sign(diffLevel);
    if targetSign == 0
        targetSign = 1;
    end
end

targetDiff = targetSign * diffLevel;
jitterRange = params.meanJitterDeg;

t1MeanDeg = baseT1Deg;
t2MeanDeg = baseT2Deg;
bestDiff = ((t2MeanDeg^expo) / (t1MeanDeg^expo)) - 1;

for attempt = 1:maxAttempts
    if attempt == round(maxAttempts * 0.5)
        jitterRange = jitterRange * 0.5;
    elseif attempt == round(maxAttempts * 0.75)
        jitterRange = jitterRange * 0.25;
    end

    jitterT1 = (2 * rand - 1) * jitterRange;
    jitterT2 = (2 * rand - 1) * jitterRange;

    candidateT1 = min(max(baseT1Deg + jitterT1, minDeg), maxDeg);
    candidateT2 = min(max(baseT2Deg + jitterT2, minDeg), maxDeg);

    currentDiff = ((candidateT2^expo) / (candidateT1^expo)) - 1;

    if currentDiff * targetSign < 0
        continue;
    end

    if abs(currentDiff - targetDiff) <= tolerance
        t1MeanDeg = candidateT1;
        t2MeanDeg = candidateT2;
        return;
    end

    if abs(currentDiff - targetDiff) < abs(bestDiff - targetDiff)
        t1MeanDeg = candidateT1;
        t2MeanDeg = candidateT2;
...
warning('jitterMeanPair:MaxAttempts', ['Could not satisfy mean diff tolerance after %d attempts. ' ...
    'Returning closest jittered pair found. Consider relaxing meanDiffTolerance or meanJitterDeg.'], maxAttempts);
end
