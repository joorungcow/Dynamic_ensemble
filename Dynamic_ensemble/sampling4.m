...
%   factors (combo × frequency ratio × diff level × repeat count).
%   It creates both main blocks (plan.blocks) and practice blocks
...
practiceDiffLevels = [0.02 0.04 0.16 0.32];
...
plan.meta.repeatCount      = repeatCount;   % backward compatibility
plan.meta.repeatMain       = repeatCount;
plan.meta.repeatPrac       = repeatCountP;
plan.meta.expo             = expo;
plan.meta.diffLevels       = diffLevels;
plan.meta.practiceDiffLevels = practiceDiffLevels;
...
for b = 1:numel(blockLetters)
    ...
    % Practice block (repeatCountP)
    % Practice block (repeatCountP, easy+hard only)
    trialsPractice = buildBlockTrials(blockLabel, comboLabels, practiceDiffLevels, ...
    freqRatios, signVals, methodPairs, repeatCountP, dotParams, gridLayout, gridConfig);
    ...
    % Main block (repeatCount)
    trialsMain = buildBlockTrials(blockLabel, comboLabels, diffLevels, ...
        freqRatios, signVals, methodPairs, repeatCount, dotParams, gridLayout, gridConfig);
...
plan.meta.NTotalWithPractice = totalTrialsMain + totalTrialsPractice;
plan.meta.N                  = totalTrialsMain;  % for compatibility with existing test scripts
...
save(saveFile, 'plan');
fprintf('Plan (main+practice) saved to %s\n', saveFile);
end

%% ========================================================================
%% ===== Helper functions (unchanged core logic from sampling.m) ==========
%% ========================================================================
...
trial.numDots = 8; % fixed

if trial.signS >= 0
    trial.standardInterval = 1;   % T1 = standard, T2 = test
else
    trial.standardInterval = 2;   % T2 = standard, T1 = test
end
end
...
trial.standardInterval = stimInfo.standardInterval;
trial.testInterval     = stimInfo.testInterval;
trial.standardMeanPS   = stimInfo.standardMeanPS;
trial.testMeanPS       = stimInfo.testMeanPS;
end
...
muStdTarget  = 1;                             % standard: 항상 1
muTestTarget = 1 + trialSpec.diffLevel;       % test: 1 + diff

% sampling3에서 standardInterval을 넣어줌.
% 혹시 없으면 signS로부터 복원 (이전 plan과 호환용)
if ~isfield(trialSpec, 'standardInterval') || isempty(trialSpec.standardInterval)
    if trialSpec.signS >= 0
        standardInterval = 1; % T2가 크다 → T1=standard, T2=test
    else
        standardInterval = 2; % T1이 크다 → T2=standard, T1=test
    end
else
...
% 실제 집합 생성 (dot-level jitter는 synthesizeSetFromBase 내부에서 처리)
...
psDiffStandardTest   = (testMeanPS / standardMeanPS) - 1;  % test vs standard
diffLevelObservedPS  = psDiffStandardTest * 100;

% info struct 채우기 (plan, test 코드 둘 다에서 사용 가능하게)
info = struct();
...
info.psDiffObserved    = psDiffStandardTest;      % 이제는 test - standard 기준
info.diffLevelObservedPS = diffLevelObservedPS;

% mean jitter는 더 이상 사용하지 않으므로 0으로 고정 (기록용 필드 유지)
info.jitterPS           = 0;
info.targetPSMeans      = [muStdTarget muTestTarget];
info.psTargetsAfterJitter = info.targetPSMeans;

% largerSide는 여전히 "어느 interval의 mean이 더 큰가 (T1 or T2)"
if testInterval == 1
    info.largerSide = 'T1';
else
    info.largerSide = 'T2';
