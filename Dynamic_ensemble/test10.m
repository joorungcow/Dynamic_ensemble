...
% ---------------------
% init_keyboard(kbStruct)
%     Expected to populate Psychtoolbox keyboard fields (e.g., key codes,
%     queue flags) and return the updated struct. The script relies on
%     kb.leftKey, kb.rightKey, kb.escKey, and optional queue initialization.
% OpenWindow(displayStruct)
%     Opens a Psychtoolbox onscreen window and returns the struct with
%     fields such as wPtr, ifi, frameRate, ppd, cx, cy. The window pointer
%     and timing values are required across drawing and timing routines.
% make_fixation(displayStruct)
%     Draws the fixation mark into the currently active window (dp.wPtr)
%     without performing a Screen('Flip'). The script assumes it is safe to
%     call each frame prior to flipping.
%
...
try
    ...
    for idx = order
        ...
        [trialResult, abortExperiment] = runTrial(idx, dp, kb, cond, dotParams, motionParams, timingParams, gridLayout, gridConfig, ratioAssignments, numDots);
...
%% Local functions -------------------------------------------------------
function [trialResult, abortExperiment] = runTrial(condIdx, dp, kb, cond, dotParams, motionParams, timingParams, gridLayout, gridConfig, ratioAssignments, numDots)
abortExperiment = false;
...
if timingParams.fixationMs > 0
    ...
    for frameIdx = 1:numFixFrames %#ok<NASGU>
        Screen('FillRect', dp.wPtr, dp.bkColor);
        make_fixation(dp);
...
stim1 = createStimulus(numDots, t1TargetMeanDeg, dotParams, gridLayout, gridConfig, t1Counts);
abortExperiment = presentStimulus(dp, stim1, t1IsMoving, motionParams, timingParams.stimDurationMs, kb);
if abortExperiment
    trialResult = struct();
    return;
end
...
stim2 = createStimulus(numDots, t2TargetMeanDeg, dotParams, gridLayout, gridConfig, t2Counts);
abortExperiment = presentStimulus(dp, stim2, t2IsMoving, motionParams, timingParams.stimDurationMs, kb);
if abortExperiment
    trialResult = struct();
    return;
end
...
for iLine = 1:numel(rawLines)
    currentLine = rawLines{iLine};
    if isnumeric(currentLine)
        instructionLines{iLine} = currentLine;
        continue;
    end
    ...
    if ischar(currentLine)
        instructionLines{iLine} = double(currentLine);
    else
        instructionLines{iLine} = double(string(currentLine));
    end
end
...
while GetSecs < deadline && ~response.wasAborted && ~response.didRespond
    ...
    for lineIdx = 1:numel(instructionLines)
        ...
        bounds = Screen('TextBounds', dp.wPtr, lineText);
        textX = dp.cx - RectWidth(bounds) / 2;
        Screen('DrawText', dp.wPtr, lineText, textX, lineY, dp.textColor);
    end
...
[keyIsDown, ~, keyCode] = KbCheck(-1);
abort = keyIsDown && any(keyCode(kb.escKey));
>>>>>>> db47bd563c8eb5a7195117b7758ad322390743d2
end